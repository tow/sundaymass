...
- The reading picker derives its allowed catalogue separately for first reading,
  responsorial psalm, second reading, and Gospel from `sunday-lectionary.json`,
  `celebrations.json`, and `commons.json`. This is more accurate than a simple Old/New
  Testament rule: the first-reading corpus includes Easter readings from Acts, while
  the psalm slot includes a small number of biblical canticles.
...
3. **`build_sunday_schedule.js`** — uses `romcal` with the **Finland** national
   calendar and applies the Finland corrections below. It writes two deliberately
   separate files:
   `sunday-calendar.json`, containing each Sunday from 2025–2075 as a lightweight
   `{date, title, season, cycle, lectionary key}` occurrence, and
   `sunday-lectionary.json`, containing each distinct A/B/C or fixed Sunday reading set
   once. The dated calendar does not duplicate reading citations.
4. **`build_celebrations.js`** — imports Proper readings, explicit `or` alternatives,
   ...
   It writes `celebrations.json` (currently 235 usable dated celebrations) and
   `commons.json` (seven Commons containing 239 role-specific reading options).
...
5. **`extract_readings.js`** — resolves each distinct citation from the unique Sunday
   ...
   Apocrypha** (`kjva.json`) for deuterocanonical books. Citations containing “or” are
   also expanded into individually selectable long/short alternatives. It preserves
   visible verse numbers in the resulting text and writes `readings_text.json`. Verse
   numbers use superscript digits followed by U+202F NARROW NO-BREAK SPACE; do not
   replace it with U+2009 THIN SPACE, which can orphan a verse number at the end of a
   line. Gaps between non-consecutive verses are rendered exactly as `[...]`.
   *(needs `web.json` + `kjva.json` — see `fetch_sources.sh`)*
6. **`make_template.js`** / **`make_template2.js`** — build the 1-page and 2-page Word
   templates (docx-js) with `@@TOKEN@@` placeholders, and unzip their parts into
   `tpl/` and `tpl2/`. *(needs `docx` npm)*
7. **`lectionary-catalog.js`** — contains the browser-side lectionary domain logic:
   ...
   specify one.
8. **`build_tool2.js`** — embeds the calendar, unique lectionary and celebration JSON
   catalogues, `lectionary-catalog.js`, `readings_text.json`, and both template part-sets
   into the final `StJames_Mass_Planner.html`. It is responsible for presentation and
   persistence, not for deciding which lectionary options are valid. The browser fills
   the Word tokens and re-zips a genuine `.docx` on download (tiny inline zip writer,
   no library).
...
- A row in `sunday-calendar.json` identifies when a celebration occurs and points to
  its lectionary entry through `l`. Calendar rows must never contain `f`, `p`, `e`, or
  `g` reading fields.
- `sunday-lectionary.json` owns the canonical scheduled citation set for each Sunday
...
node build_sunday_schedule.js
node build_celebrations.js            # network: Proper of Saints + Commons citations
node extract_readings.js
node make_template.js && node make_template2.js
node build_tool2.js
```

## Finland calendar corrections (in `build_sunday_schedule.js`)
...
- **Full text**: public-domain **World English Bible** (+ KJV Apocrypha for
  ...
  - **Psalm verse numbering** is normalized chapter by chapter from the
    ...
    `extract_readings.js` was derived by comparing the official USCCB description of
    NAB numbering, Sefaria's Hebrew chapter shapes, and the bundled WEB chapter
