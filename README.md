...
- Reading overrides are also live and public, but remain secondary. Editors choose from
  role-constrained lectionary passages, see the full text before saving, and must confirm
  non-standard selections against the parish Ordo or with the celebrant.
...
- `plans` has one row per Sunday, a JSON `choices` object, and a JSON
  `reading_overrides` object. Each Mass part stores `song`, `youtubeUrl`, `authors`,
  `copyrightOwner`, `copyrightYear`, and `source`.
...
- The reading picker derives its allowed catalogue separately for first reading,
  responsorial psalm, second reading, and Gospel from `sundays.json`. This is more
  accurate than a simple Old/New Testament rule: the first-reading corpus includes
  Easter readings from Acts, while the psalm slot includes a small number of biblical
  canticles.
...
- **Full text**: public-domain **World English Bible** (+ KJV Apocrypha for
  ...
  - The current calendar contains 645 distinct citation strings. Expanding explicit
    “or” alternatives creates 664 selectable/text entries: 643 complete, 18 partial,
    and 3 missing. The main gaps are the deuterocanonical **Daniel canticle (Prayer of
    Azariah)** and psalms whose source versification lacks a requested final verse.
  - Lettered citation fragments such as `3a`/`3b` are normalized to the whole verse by the
    current parser.
- Authorised editors can persist reading overrides. Public viewing and both Word exports
  use the saved citation and its embedded full text; adjusted readings are marked in the
...
- The reading picker opens as a full-screen dialog on mobile, keeps its action buttons
  reachable, blocks wrong-role citations, and requires the Ordo checkbox for a
