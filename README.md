...
For deployment, serve `index.html` together with `manifest.webmanifest`,
`service-worker.js`, and `icons/`. On HTTPS, the planner can be installed to a phone's
home screen and works offline after its first load.
...
node build_tool2.js        # regenerates StJames_Mass_Planner.html from included data
cp StJames_Mass_Planner.html index.html
```
...
- Deploy: drag `index.html` onto Netlify Drop / GitHub Pages (see chat for steps).
