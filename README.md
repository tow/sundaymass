...
6. **`src/domain/lectionary.js`** — contains the browser-side lectionary domain logic:
...
7. **`scripts/build-app.js`** — assembles `src/planner.html`,
   `src/styles/planner.css`, `src/app/planner.js`, the domain and print modules, and
   generated JSON catalogues into the final planner HTML. It also assembles the
   repertoire source into `repertoire.html`. It embeds the planner data but does not
...
- Authorised editors can persist a complete alternative celebration and then optionally
  fine-tune individual readings. Public viewing and both print modes use the saved
