...
// Proportional like Times, so the stub wraps text roughly where a document would and
// a flat character budget could not stand in for it.
const NARROW = " fijlrst.,;:'!-";
const WIDE = "mwABCDEFGHIJKLMNOPQRSTUVWXYZ";
const glyphWidth = glyph => (NARROW.includes(glyph) ? 300 : WIDE.includes(glyph) ? 700 : 500);
...
function stubDocument(marks) {
  ...
  return {
    ...
    getTextWidth(value) {
      return [...String(value)].reduce((sum, glyph) => sum + glyphWidth(glyph), 0)
...
// Short lines fit a narrow column; long ones cannot, which is what separates the
// songs that can afford extra columns from the songs that cannot.
const shortLines = count => verses({ count, lines: 3, text: (v, l) => `Kyrie ${v}${l}` });
const longLines = count => verses({
  count,
  lines: 4,
  text: (v, l) => `Praise to the Lord the Almighty the King of all creation ${v}${l}`,
});
const songs = (count, build) => Array.from({ length: count }, (_, index) => assignment({
...
// Imposition puts logical page 8 and 1 on the first sheet, 2 and 7 on the next, and
// so on, so a drawn mark's sheet and side identify the page it landed on.
const SLOTS = {
  "0:left": 8, "0:right": 1, "1:left": 2, "1:right": 7,
  "2:left": 6, "2:right": 3, "3:left": 4, "3:right": 5,
};

// Builds the booklet and returns the lowest point drawn on each logical page.
function paintedDepths(options) {
  const marks = [];
  ...
  marks.forEach(mark => {
    ...
    depths.set(slot, Math.max(depths.get(slot) || 0, bottom));
  });
  return depths;
}
...
test("no column ends with a label and every column receives content", () => {
  const result = LyricsBooklet.paginateLyrics(
    songs(3, () => ({ lyrics: shortLines(18) })), { measure: stubMeasure() },
  );
...
test("column count follows line length rather than song length", () => {
  ...
  const shortLined = LyricsBooklet.paginateLyrics(
    [assignment({ title: "Short Lined", lyrics: shortLines(13) })], { measure },
  );
  const longLined = LyricsBooklet.paginateLyrics(
    [assignment({ title: "Long Lined", lyrics: longLines(10) })], { measure },
  );
...
test("a page uses the full printable height rather than a fixed line budget", () => {
  const result = LyricsBooklet.paginateLyrics(
    [assignment({ title: "Huge", lyrics: longLines(30) })], { measure: stubMeasure() },
  );
...
test("long lyrics continue on later logical pages without overflowing capacity", () => {
  const result = LyricsBooklet.paginateLyrics(
    [assignment({ lyrics: longLines(24) })], { measure: stubMeasure() },
  );
...
test("an oversized song continues across pages under a continued header", () => {
  const result = LyricsBooklet.paginateLyrics(songs(6, index => ({
    lyrics: index === 2 ? longLines(40)
...
test("painted content never runs past the reservation or the printable area", () => {
  const shapes = {
    mixed: songs(8, index => ({ lyrics: longLines((index % 3) + 2) })),
    huge: [assignment({ title: "Huge", lyrics: longLines(30) })],
    tiny: songs(3, () => ({ lyrics: shortLines(18) })),
  };

  Object.entries(shapes).forEach(([name, assignments]) => {
    const options = { date: "2026-08-02", celebration: "18th Sunday", meta: "Year A", assignments };
    const painted = paintedDepths(options);

    LyricsBooklet.logicalPages({ ...options, measure: stubMeasure() })
      .filter(page => page.kind === "lyrics")
      .forEach(page => {
        ...
        assert.ok(drawn <= reserved + 0.01,
          `${name} p${page.number}: painted ${drawn.toFixed(2)} > reserved ${reserved.toFixed(2)}`);
        assert.ok(drawn <= PRINTABLE_BOTTOM,
          `${name} p${page.number}: painted ${drawn.toFixed(2)} past ${PRINTABLE_BOTTOM}`);
      });
  });
});
